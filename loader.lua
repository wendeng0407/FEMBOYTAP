...
if not src then print("[加载器] 致命错误：无法获取 femboytap.lua") return end

local chunk, err = loadstring(src, "=femboytap.lua")
if not chunk then print("[加载器] 编译错误：" .. tostring(err)) return end

_G.FEMBOY_BASE = BASE
print(string.format("[加载器] femboytap %s 来源：%s", ref(), tostring(where)))

local ok, e = pcall(chunk)
if not ok then print("[加载器] 运行错误：" .. tostring(e)) end