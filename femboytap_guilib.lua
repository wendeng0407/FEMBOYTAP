--[[
    file: femboytap_guilib.lua
    汉化内容: 界面标题、按钮、标签、提示、水印、命中日志、通知等所有用户可见文字
]]
local M = {}
M.VERSION = "1.0"
...
local function fontInitCoro()
    ...
    if gdi32 then
        ...
        print("[喵毒药] ffi/gdi32 不可用，使用系统字体")
    end
...
function Section:_widget(wd, x, y, w)
    ...
    elseif wd.kind == "button" then
        ...
        if clicked(x, y + 1, w, bh) then
            local ok, err = pcall(wd.cb); if not ok then print("[喵毒药] 按钮错误：" .. tostring(err)) end
        end
    ...
    elseif wd.kind == "multicombo" then
        ...
        local shown = count == 0 and "无" or (count > 2 and (count .. " 项已选") or table.concat(parts, ", "))
    ...
    elseif wd.kind == "custom" then
        if wd.fn then
            ...
            if not ok then print("[喵毒药] 自定义控件错误：" .. tostring(err)) end
            local used = UI._cy - y
...
local function renderSectionAt(s, x, y, w)
    ...
    if not ok then print("[喵毒药] 区块 '" .. tostring(s.title) .. "' 错误：" .. tostring(err)); return h end
    return rh
end
...
local HITLOG_TEXT = { miss = "未命中", hit = "命中", hurt = "受伤", kill = "击杀敌人" }
...
local HITLOG_DEMO = {
    { kind = "hit",  label = "击中玩家头部 90 伤害" },
    { kind = "hurt", label = "被玩家击中胸部 20 伤害" },
    { kind = "miss", label = "未命中" },
    { kind = "kill", label = "击杀玩家 100 伤害" },
}
...
function M:_drawWatermark()
    ...
    if wm.parts.lua   then segs[#segs + 1] = nameSeg(wm.lua_name or "喵毒药.CC") end
...
function M:_frame()
    ...
    if tab then
        ...
        if not ok then print("[喵毒药] 标签页 '" .. tostring(tab.name) .. "' 错误：" .. tostring(err)) end
    end
...
function M:_initScreen()
    ...
    text(win.x + win.w / 2, win.y + win.h / 2 - 12, T.texthi, "初始化中" .. dots, FONT_B, "center")
    text(win.x + win.w / 2, win.y + win.h / 2 + 12, T.textdim, "正在获取字体，请稍候", FONT, "center")
end
...
function M:Build(opts)
    ...
    if not _getMouse then print("[喵毒药] 警告：未找到鼠标位置API —— 光标将无法跟踪") end
    ...
    callbacks.Register("Draw", function()
        ...
        if not ok then print("[喵毒药] 帧错误：" .. tostring(err)) end
    end)
    ...
    print(string.format("[喵毒药.cc] GUI库 v%s 就绪：共 %d 个标签页，鼠标=%s 时钟=%s",
        tostring(M.VERSION), #self._tabs, _getMouse and "正常" or "缺失", _clock and "正常" or "缺失"))
    return self
end
